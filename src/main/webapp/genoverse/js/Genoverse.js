...
var Genoverse = Base.extend({
  ...
  init: function () {
    ...
    this.selectorControls = $('                      \
      <div class="selector_controls">                \
        ...
        <button class="viewdata">View data</button>     \
    ...
    //r=__CHR__:__START__-__END__
    var urlcoords = this.getCoords();
    ...
    this.setChrAndRange(coords.chr, coords.start, coords.end);
    this.setTracks();
    this.setHistory();
    this.makeImage();
  ...
  addUserEventHandlers: function () {
      ...
      mousewheel: function (e, delta, deltaX, deltaY) {
        if (browser.wheelAction === 'zoom') {
          ...
          if(deltaY === 0 && deltaX !== 0) {
            if (browser.dragAction === 'scroll') {
              browser.startDragScroll(e);
              browser.move(null, -deltaX * 10);
              browser.stopDragScroll(e, false);
            }
          }
        }
      }
    }, '.image_container, .overlay, .selector, .track_message .track_controls');
    ...
    this.selectorControls.on('click', function (e) {
      ...
      switch (e.target.className) {
        ...
        case 'viewdata' : browser.viewDataRange(start, end); break;
  ...
  viewDataRange: function (start, end) {
    var ntracks = this.tracks.length;
    var bounds = { x: start, y: 0, w: end - start, h: 1 };
    var i = 0;
    var data = '<div><textarea cols="150" rows="50">';

    for (i = 0; i < ntracks;i++) {
      if (typeof this.tracks[i].printData !== 'undefined') {

          //var bufferedStart = Math.max(start - (this.labelOverlay ? 0 : this.browser.labelBuffer), 1);
          var features = this.tracks[i].features.search(bounds);
          data += this.tracks[i].printData(features);
      }
    }
    ...
    var menu = this.makeMenu(viewMenu);
  ...
  resetBrowser: function () {
     var i = this.tracks.length;
              
     while (i--) {
         if (typeof this.tracks[i].reset !== 'undefined') {
             this.tracks[i].reset();
         }
     }
              
 ...
  },
  ...
  setRange: function (start, end, update, force) {
      this.setChrAndRange(this.chr, start, end, update, force);
  },
  ...
  setScale: function (force) {
    ...
    if (force || this.prev.scale !== this.scale) {
      ...
      if (this.prev.scale) {
        ...
        while (i--) {
          if (typeof this.tracks[i].setScale !== 'undefined') {
  ...
  */
  ...
  makeMenu: function (feature, position, track) {
    ...
    var menu    = this.menuTemplate.clone(true).data('browser', this).appendTo($('body'));
    ...
    $.when(track ? track.populateMenu(feature) : feature).done(function (feature) {
      ...
      // allow menus to access browser
      // instance if necessary, not sure
      // if a security breach of some sort
      // is possible
      //var browser = this;

      var dpos = { top  : offset.top  + 150,
                   left : offset.left + (wrapper.outerWidth(true) - menu.outerWidth(true))/2 }

      if (position) {
          position['top'] = position['top'] || dpos.top;
          position['left'] = position['left'] || dpos.left;
      }
      ...
      obj[key] = function () {
        ...
        if (this.systemEventHandlers['before' + func]) {
          for (i = 0; i < this.systemEventHandlers['before' + func].length; i++) {
            // TODO: Should it end once beforeFunc returned false or something??
        ...
        if (this.systemEventHandlers['after' + func]) {
          for (i = 0; i < this.systemEventHandlers['after' + func].length; i++) {
            // TODO: Should it end once afterFunc returned false or something??
...
Genoverse.on('afterMove afterStopDragScroll afterZoomIn afterZoomOut', function () {
