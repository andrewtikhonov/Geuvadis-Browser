Genoverse.on('beforeInit', function () {
  ...
  $('td.genoverse_panel_right').append('\
    ...
    <div class="button_set">\
    <button class="dragScroll" title="Dragging">&#8596;</button><button class="dragSelect" title="Region selection and study">&#9482;</button>\
    </div>\
    <div class="button_set">\
    <button class="wheelOff" title="Use mouse wheel for scrolling">&#8597;</button><button class="wheelZoom" title="Use mouse wheel for zooming">&#177;</button>\
    </div>\
    <div class="button_set">\
    <button class="trackSelection" title="Select tracks" style="width: 100%">...</button>\
    </div>\
  ');
  ...
  if (browser.enableSharing) {
      $('td.genoverse_panel_right').append('\
        <div class="button_set">\
        <button class="share" title="Share the link to the region" style="width: 100% ">share</button>\
        </div>\
      ');
  }

  if (browser.enableDataView) {
      $('td.genoverse_panel_right').append('\
        <div class="button_set">\
        <button class="view" title="Select region and click view data" style="width: 100% ">view</button>\
        </div>\
      ');
  }
  ...
  if (browser.enableSharing) {
    $(".genoverse_panel button.share").click(function(e){
  ...
  if (browser.enableDataView) {
      $(".genoverse_panel button.view").click(function(){
          browser.setDragAction('select');
          $(".genoverse_panel button.dragSelect").addClass('active');
          $(".genoverse_panel button.dragScroll").removeClass('active');
          ...
          var viewMenu = {
              title  : 'View data:',
              ' '    : "Select region and click \"view data\""
          };

          var menu = browser.makeMenu(viewMenu);
          menu.addClass('track_info');
          */
      });
  }
